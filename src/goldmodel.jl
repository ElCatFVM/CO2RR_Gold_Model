...
function create_model(;
    ...
    function pnp_bcondition(
        ...
        if bnode.region == Γ_we
            ...
            # IR-compensation operator sets the (compensated) working-electrode voltage,
            # so we must NOT set it again here — do it explicitly only for :none.
            # (Mirrors `halfcellbc` in the IRCompensationRobin example.)
            ...
            # Evaluated here except for :ohmicdrop, where the generic operator evaluates
            # them (it needs the faradaic current for the ohmic-drop estimate). The
            # reaction is registered on the electrolyte as `redoxreaction = we_breactions`
            # so the operator can call it.
    ...
    # Build the electrolyte here (AFTER `we_breactions` exists) so the surface reaction
    # is registered at CONSTRUCTION time. ElectrolyteData's `redoxreaction` field is
    # concretely typed and cannot be reassigned afterwards — it must go through the
    # constructor. The IR-compensation operator uses it to evaluate the faradaic current
    # in :ohmicdrop / :pseudopotentiostat modes.
    if model == :Gold
