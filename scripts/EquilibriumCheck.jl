...
function plot_combined_exp_sim_ivc(
    P_recs;
    redox_species = Dict(ico => 1),  # OH- 1개당 1 electron (CO2RR/HER 둘 다)
    include_capacitive = true,
    scale = cm^2/mA,
...
if unc
	pnpresult_unc = sweep(elydata_Gold_unc, grid, pnp_bcondition, reaction, sawtooth_v2; nperiods = user_input_cv.nperiods, eneutral = true, tunnel = false)
end
...
odr_L_result = cvsweep_odr_over_L(elydata_Gold_odr, grid_dict, pnp_bcondition,  reaction, sawtooth; nperiods = 1)
